...
jupiterGroup.rotation.z = -3.1 * Math.PI / 180; // tilt to match real world
...
jupiterGroup.add(jupiterMesh); //Why not there? not infornt of caerma, no lights in scene, or mesh not added to scene
...
const jupiterLight = new THREE.DirectionalLight(0xffffff);// color, intensity
jupiterLight.position.set(-2,0.5,1.5); //
scene.add(jupiterLight);

function animate() {
    requestAnimationFrame(animate);
    
    jupiterMesh.rotation.y += 0.024;
