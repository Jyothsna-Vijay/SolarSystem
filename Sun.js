...
const h = window.innerHeight; //NOT HAPPY WITH FINAL SUN
...
sunGroup.add(sunMesh); //Why not there? not infornt of caerma, no lights in scene, or mesh not added to scene
...
/*const sunLight = new THREE.PointLight(0xffffff, 5, 0, 2); // color, intensity, distance, decay
// distance = 0 means infinite, decay=2 = inverse-square law
sunLight.position.set(0, 0, 0); // Sun at origin
scene.add(sunLight);

// also make the Sun visible (optional)
const sunGeo = new THREE.SphereGeometry(16, 30, 30);
const sunMat = new THREE.MeshBasicMaterial({
  map: textureLoader.load(sunTexture),
  emissive: 0xffffaa, // glows a bit
  emissiveIntensity: 1
});
const sunlightMesh = new THREE.Mesh(sunGeo, sunMat);
scene.add(sunlightMesh);*/

//const sunLight = new THREE.DirectionalLight(0xffffff);// color, intensity
//sunLight.position.set(-2,0.5,1000); //
//scene.add(sunLight);
